//! Benchmarks for the cost of verifying [`SolRegistration`]s and [`SolSignature`]s, and the helpers that
//! produce valid ones.
//!
...
//! the authority and challenge checks, building the signed message, and checking its Ed25519 signature (through the host). Inputs have a
...
pub mod helpers {
    ...
    const PREFIX: &[u8] = b":pass-authenticators:solana:benchmarks:";
    ...
    fn public(seed: &[u8; 32]) -> SolPubkey {
        SolPubkey(
    ...
    /// Returns a valid [`SolRegistration`] of a freshly generated key, answering `challenge`
    ...
    /// Returns a valid [`SolSignature`] for `user_id`, answering `challenge` (generated from
...
impl<Cx: Parameter + 'static> DeviceAttestationBenchmarkHelper<Cx> for SolRegistration<Cx> {
...
impl<Cx: Parameter + 'static> CredentialBenchmarkHelper<Cx> for SolSignature<Cx> {
...
fn device_and_credential() -> (BenchDevice, SolSignature<u32>) {
